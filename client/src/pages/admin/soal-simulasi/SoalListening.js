...
const SoalListening = () => {
  ...
  const fetchSoalListening = async () => {
    Swal.fire({
      title: "Memuat data...",
      ...
      <div className="relative overflow-auto">
        <table className="border-collapse min-w-full border border-slate-300 bg-white text-sm shadow-sm">
          ...
          <tbody>
            {soals.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center py-6 text-gray-400">Belum ada soal listening</td>
              </tr>
            ) : (
              soals.map((soal, idx) => (
                <tr key={soal.id}>
                  ...
                  <td className="border px-4 py-2 font-bold">
                    <div className='flex justify-center gap-2'>
                      <button
                      onClick={() => navigate(`/admin/soal-simulasi/edit-soal-listening/${soal.id}`, {
